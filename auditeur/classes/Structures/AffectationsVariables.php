...
class Structures_AffectationsVariables extends modules {
...
FROM <tokens> T1
JOIN <tokens> T2
    ON T1.file = T2.file AND T2.left = T1.left + 1 AND T2.type = 'functioncall'
JOIN <tokens> T3
    ON T1.file = T3.file AND T3.left = T1.left + 2 AND T3.type = 'token_traite' AND T3.code = 'list'
JOIN <tokens> T4
